...
func TestChannelCP(t *testing.T) {
	...
	t.Run("ListChannelCheckpoint", func(t *testing.T) {
		...
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return([]string{k}, []string{string(v)}, nil)
	...
	t.Run("ListChannelCheckpoint failed", func(t *testing.T) {
		txn := mocks.NewMetaKv(t)
		catalog := NewCatalog(txn, rootPath, "")
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return(nil, nil, errors.New("mock error"))
	...
	t.Run("DropChannelCheckpoint", func(t *testing.T) {
		...
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return(nil, nil, nil)
...
func TestCatalog_ListIndexes(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		metakv := mocks.NewMetaKv(t)
		metakv.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).RunAndReturn(func(ctx context.Context, s string) ([]string, []string, error) {
	...
	t.Run("failed", func(t *testing.T) {
		txn := mocks.NewMetaKv(t)
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return(nil, nil, errors.New("error"))
		catalog := &Catalog{
			MetaKv: txn,
		}
	...
	t.Run("unmarshal failed", func(t *testing.T) {
		txn := mocks.NewMetaKv(t)
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return([]string{"1"}, []string{"invalid"}, nil)
...
func TestCatalog_ListSegmentIndexes(t *testing.T) {
	t.Run("success", func(t *testing.T) {
		...
		metakv := mocks.NewMetaKv(t)
		metakv.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return([]string{"key"}, []string{string(v)}, nil)
		catalog := &Catalog{
			MetaKv: metakv,
		}
	...
	t.Run("failed", func(t *testing.T) {
		metakv := mocks.NewMetaKv(t)
		metakv.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return([]string{}, []string{}, errors.New("error"))
		catalog := &Catalog{
			MetaKv: metakv,
		}
	...
	t.Run("unmarshal failed", func(t *testing.T) {
		metakv := mocks.NewMetaKv(t)
		metakv.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return([]string{"key"}, []string{"invalid"}, nil)
		catalog := &Catalog{
			MetaKv: metakv,
		}
...
func TestCatalog_Import(t *testing.T) {
	...
	t.Run("ListImportJobs", func(t *testing.T) {
		...
		assert.NoError(t, err)
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return(nil, []string{string(value)}, nil)
		kc.MetaKv = txn
		...
		txn = mocks.NewMetaKv(t)
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return(nil, []string{"@#%#^#"}, nil)
		kc.MetaKv = txn
		_, err = kc.ListImportJobs(context.TODO())
		assert.Error(t, err)

		txn = mocks.NewMetaKv(t)
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return(nil, nil, mockErr)
		kc.MetaKv = txn
		_, err = kc.ListImportJobs(context.TODO())
		assert.Error(t, err)
	})
...
func TestCatalog_AnalyzeTask(t *testing.T) {
	...
	t.Run("ListAnalyzeTasks", func(t *testing.T) {
		...
		txn = mocks.NewMetaKv(t)
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return([]string{"key1"}, []string{
			string(value),
		}, nil)
		kc.MetaKv = txn
		...
		txn = mocks.NewMetaKv(t)
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return([]string{"key1"}, []string{"1234"}, nil)
		kc.MetaKv = txn
...
func Test_PartitionStatsInfo(t *testing.T) {
	...
	t.Run("ListPartitionStatsInfo", func(t *testing.T) {
		...
		txn = mocks.NewMetaKv(t)
		txn.EXPECT().LoadWithPrefix(mock.Anything, mock.Anything).Return([]string{"key1"}, []string{string(value)}, nil)
		kc.MetaKv = txn
