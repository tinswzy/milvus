	...
	for _, msg := range msMsg.TsMessages() {
		...
		case commonpb.MsgType_AlterWAL:
			...
			log.Info("SWITCH_MQ_STEPS: receive switch mq message",
				zap.String("pchannel", alterWALMsg.AlterWALMessage.VChannel()), // TODO:COMMENT_TO_REMOVE pchan广播消息中，msg里面放的是pchannel 名字
				zap.String("vchannel", ddn.vChannelName),
				zap.String("targetMq", alterWALMsg.AlterWALMessage.Header().TargetMq),
			...
			ddn.msgHandler.HandleAlterWAL(alterWALMsg.AlterWALMessage, ddn.vChannelName)
		}
	}
